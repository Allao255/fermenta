...
netlist = 'DEMO';   % change to any one_non_lin circuit (e.g. 'dap_clipper')
outNode = 'N003';   % and its output node
fs      = 48000;  dur = 0.05;
...
for s = 1:numel(names)
    ...
    for ii=1:N
        ...
        if typeOrder(posD) == 10
            b(posD) = antiExtSchockleyDiodeScat(S(posD,:)*b, Z(posD), ...
